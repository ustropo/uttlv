...
class BaseTLV(abc.ABC):
    ...
    tag: BaseTag
    __real_value: bytes = None
    __converted_value: Any = None
    ...
    def value(self) -> Any:
        ...
        :returns: :py:attr:`BaseTLV.__converted_value` if available.
                  Otherwise, it returns  :py:attr:`BaseTLV.__real_value`.
        """
    ...
    def set_value(self, new_value: Any) -> None:
        ...
        If `new_value` is `bytes`, then the value is set to :py:attr:`BaseTLV.__real_value`. If this
        tag has a known type, and a tag is available, it is then converted to that particular
        type and the value set to :py:attr:`BaseTLV.__converted_value`.

        If `new_value` is of any type other than `bytes`, and a tag is available, then it tries
        to encode the value into `bytes` and set it to :py:attr:`BaseTLV.__real_value`. The original
        value is set to :py:attr:`BaseTLV.__converted_value`.

        :param new_value: new value of this TLV object
        """
        self.__real_value, self.__converted_value = self._convert_value(new_value)
